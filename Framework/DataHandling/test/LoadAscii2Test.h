...
private:
  ...
  std::string writeTestFile(const int cols, const bool header = true,
    ...
    if (cols < 3) {
      ...
      for (int i = 0; i < 5; i++) {
        ...
        if (cols == 4)
          wsToSave->histogram(i).setPointStandardDeviations(4, 1.0);
      }
  ...
  void checkData(const Mantid::API::MatrixWorkspace_sptr outputWS,
    ...
    if (cols == 4) {
      TS_ASSERT_DELTA(outputWS->histogram(0).dx()[0], 1, 1e-6);

      TS_ASSERT_DELTA(outputWS->histogram(0).dx()[1], 1, 1e-6);

      TS_ASSERT_DELTA(outputWS->histogram(1).dx()[2], 1, 1e-6);

      TS_ASSERT_DELTA(outputWS->histogram(3).dx()[3], 1, 1e-6);
    }
  }
