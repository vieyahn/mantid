...
public:
  ...
  void test_Four_Column_Example_With_No_Header() {
    ...
    TS_ASSERT_EQUALS(outputWS->histogram(0).dx()[0], 0.1);
    TS_ASSERT_EQUALS(outputWS->histogram(0).dx()[18], 1.9);
    TS_ASSERT_EQUALS(outputWS->histogram(0).dx()[29], 0.8);
    Poco::File(filename).remove();
  }
...
private:
  ...
  void checkData(const Mantid::API::MatrixWorkspace_sptr outputWS,
    ...
    if (threeColumn) {
      ...
      TS_ASSERT(!outputWS->histogram(0).sharedDx());
